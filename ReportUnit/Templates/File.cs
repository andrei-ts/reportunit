...
<html lang='en'>
	<head>
		...
		<link href='https://cdn.rawgit.com/andrei-turavets-epam/reportunit/21f48e8ac785f1178fb2324d32eae97a77e2abe5/ReportUnit/cdn/reportunit.css' type='text/css' rel='stylesheet' />
							...
							<div class='card-panel no-padding no-margin-v suite-list v-spacer'>
								...
								<ul id='suite-collection' class='no-margin-v'>
									<ul class='collapsible' data-collapsible='accordion'>
										@for (var ip = 0; ip < Model.ParentTestSuiteList.Count; ip++)
										{
											...
											<li>
												...
												<div class='collapsible-body'>
													...
													{
														<div class='suite @parentSuite.TestSuiteList[ix].Status.ToString().ToLower()'>
															<div class='suite-head'>
																<div class='suite-name'>@parentSuite.TestSuiteList[ix].Name</div>
																@if(@parentSuite.TestSuiteList[ix].Status.ToString().ToLower() == ""failed"" && @parentSuite.TestSuiteList[ix].TestList.Count(x=> x.Status.ToString().ToLower()== ""failed"") != @parentSuite.TestSuiteList[ix].TestList.Count)
																{
																	 <div class='suite-result @parentSuite.TestSuiteList[ix].Status.ToString().ToLower() right label'>@parentSuite.TestSuiteList[ix].TestList.Count(x=> x.Status.ToString().ToLower()== ""failed"") of @parentSuite.TestSuiteList[ix].TestList.Count @parentSuite.TestSuiteList[ix].Status.ToString()</div>
																}
																else
																{
																	<div class='suite-result @parentSuite.TestSuiteList[ix].Status.ToString().ToLower() right label'>@parentSuite.TestSuiteList[ix].Status.ToString()</div>
																}
															</div>
															<div class='suite-content hide'>
																<span alt='Suite started at time' title='Suite started at time' class='startedAt label green lighten-2 text-white'>@parentSuite.TestSuiteList[ix].StartTime</span>
																@if (!String.IsNullOrEmpty(@parentSuite.TestSuiteList[ix].EndTime))
																{
																	<span alt='Suite ended at time' title='Suite ended at time' class='endedAt label label red lighten-2 text-white'>@parentSuite.TestSuiteList[ix].EndTime</span>
																}
																<div class='fixture-status-message'>
																	@if (!String.IsNullOrEmpty(@parentSuite.TestSuiteList[ix].Description)) 
																	{
																		<div class='suite-desc'>@parentSuite.TestSuiteList[ix].Description</div>
																	}
																	@if (!String.IsNullOrEmpty(@parentSuite.TestSuiteList[ix].StatusMessage)) 
																	{
																		<div class='suite-desc'>@parentSuite.TestSuiteList[ix].StatusMessage</div>
																	}
																</div>
																<table class='bordered'>
																	<thead>
																		<tr>
																			...
																			<th>Test Method</th>
																			<th>Status</th>
																			@if (parentSuite.TestSuiteList.Count > 0 && parentSuite.TestSuiteList[ix].TestList.Any(x => x.CategoryList.Count > 0))
																			{
																				<th>Category</th>
																			}
																			@if (parentSuite.TestSuiteList.Count > 0 && parentSuite.TestSuiteList[ix].TestList.Where(x => !String.IsNullOrEmpty(x.Description) || !String.IsNullOrEmpty(x.StatusMessage)).Count() > 0) 
																			{
																				<th>Logs</th>
																			}
																			<th>Screenshots</th>
																	...
																	<tbody>
																		@foreach (var test in parentSuite.TestSuiteList[ix].TestList)
																		{
																			<tr class='@test.Status.ToString().ToLower() test-status'>
																				...
																				@if (parentSuite.TestSuiteList.Count > 0 && parentSuite.TestSuiteList[ix].TestList.Where(x => !String.IsNullOrEmpty(x.StatusMessage)).Count() > 0) 
																				{
	...
	<script src='https://cdn.rawgit.com/andrei-turavets-epam/reportunit/21f48e8ac785f1178fb2324d32eae97a77e2abe5/ReportUnit/cdn/reportunit.js' type='text/javascript'></script>
